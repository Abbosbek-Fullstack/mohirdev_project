...
class HomePageView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['mahalliy_xabarlar'] = News.published.all().filter(category__name="Mahalliy").order_by('-publish_time')[1:5]
        context['xorij_xabarlari'] = News.published.all().filter(category__name="Xorij").order_by('-publish_time')[1:6]
        context['sport_xabarlari'] = News.published.all().filter(category__name="Sport").order_by('-publish_time')[1:5]
        context['trenddagi_xabarlar'] = News.published.all().filter(category__name="Trend").order_by('-publish_time')[1:6]
        context['texnologiya_xabarlari'] = News.published.all().filter(category__name="Texnologiya").order_by('-publish_time')[1:5]
...
class TrendingNewsView(ListView):
    model = News
    template_name = 'news/trend.html'
    context_object_name = 'trenddagi_yangiliklar'

    def get_queryset(self):
        news = self.model.published.all().filter(category__name="Trend")
        return news
